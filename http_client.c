...
int send_activity(const char* activity, const char* auth_header, request_rec *r, CURL *curl) {
    if (curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_URL, ACTIVITIES_URL);
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, activity);
        res = curl_easy_perform(curl);

        if (res == CURLE_OK) {
            curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &code);
            if (code != 200) {
                ERROR(r->server, "Activity send error, http code: %ld", code);
                return REQ_FAILED;
            }
        }
    ...
    ERROR(r->server, "cURL handle is NULL");
    return REQ_FAILED;
}
...
//debug
void print_response(risk_response *rr) {
    if (!rr) {
        printf("This is null");
    }
    printf("uuid:%s\nstatus:%d\nscore:%d\n\n", rr->uuid, rr->status, rr->score);
}
...
char* risk_api_request(const char *risk_payload, const char *auth_header, request_rec *r, CURL *curl) {
    ...
    if (curl) {
        ...
        headers = curl_slist_append(headers, JSON_CONTENT_TYPE);
        headers = curl_slist_append(headers, EXPECT_HEADER);
        curl_easy_setopt(curl, CURLOPT_URL, RISK_API_URL);
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, risk_payload); // post goes here
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_response_cb); // post goes here
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void*) &response); // post goes here
        res = curl_easy_perform(curl);
        ...
        curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &code);
        if (code != 200) {
            ERROR(r->server, "risk api failed: , status: %ld, body: %s", code, response.data);
            free(response.data);
            curl_slist_free_all(headers);
            return NULL;
        }
